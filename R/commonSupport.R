getCommonSupportSubset <- function(sd.obs, sd.cf, commonSup.rule, commonSup.cut, trt)
{
  ...
  cut  <- getCommonSupportCutoff(sd.obs, sd.cf, commonSup.rule, commonSup.cut, trt)
...
getCommonSupportCutoff <- function(sd.obs, sd.cf, commonSup.rule, commonSup.cut, trt)
  switch(commonSup.rule,
         sd = c(trt = max(sd.obs[trt > 0]), ctl = max(sd.obs[trt <= 0])) + commonSup.cut * sd(sd.obs),
