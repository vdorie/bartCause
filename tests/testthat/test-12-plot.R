...
test_that("plot_sigma overlays one trace per chain (dbarts 1.0-0 flattens sigma/first.sigma)", {
  ...
  ## chain-major vectors even when n.chains > 1 (no per-chain matrix), so the
  ...
  sigma.mat <- matrix(fit$fit.rsp$sigma, nrow = n.chains, byrow = TRUE)
  warmup.mat <- matrix(fit$fit.rsp$first.sigma, nrow = n.chains, byrow = TRUE)
