...
test_that("bart fit matches manual call", {
  ...
  expect_equal(bartFit$yhat.train, cibartFit$fit$yhat.train)
  expect_equal(bartFit$yhat.test,  cibartFit$fit$yhat.test)
})
...
test_that("pweight fits", {
  ...
  testCall <- quote(cibart(y, z, x, testData, method.trt = "glm", method.rsp = "pweight",
                    n.chains = 1L, n.threads = 1L, n.samples = 200L, n.burn = 40L, verbose = FALSE))
  
  expect_is(eval(testCall), "cibartFit")
